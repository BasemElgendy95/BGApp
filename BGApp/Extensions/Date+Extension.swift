...
extension Date {
    ...
    func timeAgoSinceDate() -> String {
        ...
        if let interval = Calendar.current.dateComponents([.year], from: fromDate, to: toDate).year, interval > 0  {
            ...
            return interval == 1 ? "\(interval)" + " " + "منذ عام" : "\(interval)" + " " + "أعوام"
        }
        
        // Month
        if let interval = Calendar.current.dateComponents([.month], from: fromDate, to: toDate).month, interval > 0  {
            
            return interval == 1 ? "\(interval)" + " " + "منذ شهر" : "\(interval)" + " " + "شهور"
        }
        
        // Day
        if let interval = Calendar.current.dateComponents([.day], from: fromDate, to: toDate).day, interval > 0  {
            
            return interval == 1 ? "\(interval)" + " " + "منذ يوم" : "\(interval)" + " " + "ايام"
        }
        
        // Hours
        if let interval = Calendar.current.dateComponents([.hour], from: fromDate, to: toDate).hour, interval > 0 {
            
            return interval == 1 ? "\(interval)" + " " + "منذ ساعة" : "\(interval)" + " " + "ساعات"
        }
        
        // Minute
        if let interval = Calendar.current.dateComponents([.minute], from: fromDate, to: toDate).minute, interval > 0 {
            
            return interval == 1 ? "\(interval)" + " " + "منذ دقيقة" : "\(interval)" + " " + "دقائق"
        }
