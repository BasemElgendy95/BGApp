...
class BGStarRatingView: UIView {
    ...
    fileprivate var hstack: BGStarRatingStackView?
    ...
    fileprivate func setupView(rating: Float, color: UIColor, starRounding: StarRounding) {
        let bundle = Bundle(for: BGStarRatingStackView.self)
        let nib = UINib(nibName: "BGStarRatingStackView", bundle: bundle)
        let viewFromNib = nib.instantiate(withOwner: self, options: nil)[0] as! BGStarRatingStackView
